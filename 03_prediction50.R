...
save.image(file='../temp/prediction50.RData')
...
resuts_collected %>% saveRDS('../temp/resuts_collected50.rds')
...
library(rpart.plot) 
setEPS()
postscript("../output/fig_structure_dt.eps")
fit_dt$fit$fit$fit %>% rpart.plot()
dev.off()

# NEural network
library(NeuralNetTools)

pdf("../output/fig_structure_nn.pdf")
fit_nn$fit$fit$fit %>% plotnet(alpha=0.75)
dev.off()
